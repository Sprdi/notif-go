  ...
  const handleDeleteNotification = (id) => {
    axios.delete(`http://localhost:8080/notifications/${id}`)
      .then(() => {
        ...
        setNotification(Notification.filter(event => event.ID !== id));
      })
      .catch((error) => {
                ...
                return (
                  <Dropdown.Item key={event.ID}>
                    <span className="block text-sm truncate">
                      {event.title} - {formattedStart}
                    </span>
                    <button onClick={() => handleDeleteNotification(event.ID)} className="text-red-500 hover:text-red-700">
                      Delete
                    </button>
                  </Dropdown.Item>
                );
